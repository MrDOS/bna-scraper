...
BNA_URL_PATTERN = re.compile(
    r'^https?://(www.)?britishnewspaperarchive.co.uk/viewer(/items)?/(?P<page>(?P<issue>\w+/\d+/\d+)/\d+/(?P<page_number>\d+))'
)
...
class RetrievalJob(namedtuple('RetrievalJob', ['url', 'page_ranges'])):
    pass
...
Pages from more than one publication can be retrieved in a single script
invocation by populating a “batch file” and passing it via the “-b, --batch”
option. The file must contain one publication URL per line, optionally followed
by a whitespace-separated page range specification. For example,

    https://www.britishnewspaperarchive.co.uk/viewer/bl/0002268/18650211/040/0019
    https://www.britishnewspaperarchive.co.uk/viewer/bl/0002408/18550125/008/0001 all
    https://www.britishnewspaperarchive.co.uk/viewer/bl/0003073/18771110/025/0003 1-3,5
...
Formally, the default output format is:
   ...
   {DEFAULT_OUTPUT_FORMAT}
    ...
    parser.add_argument('-b',
                        '--batch',
                        dest='batch_file',
                        help='multiple URLs and page ranges to retrieve')
    ...
    parser.add_argument('url',
                        nargs='?',
                        help='the URL of the publication')

    args = parser.parse_args()

    if args.batch_file is None and args.url is None:
        parser.error('Either a URL or a batch file must be given!')
    elif args.batch_file is not None and args.url is not None:
        parser.error('A URL and a batch file cannot both be given!')
    elif args.page_ranges is not None and args.batch_file is not None:
        parser.error('Batch page ranges may only be given in the batch file!')
    ...
    session.headers.update({'user-agent': 'bna-scraper'})

    if args.batch_file is not None:
        jobs = load_jobs(args.batch_file)
    else:
        jobs = [RetrievalJob(args.url, args.page_ranges)]

    for job in jobs:
        fetch_publication(job, args.output_format, args.debug, session)


def load_jobs(batch_file):
    with open(batch_file, 'r') as batch:
        jobs = []
        for line in batch:
            url, page_ranges = (line.strip().split() + [None] * 2)[:2]
            if url is not None:
                jobs.append(RetrievalJob(url, page_ranges))
        return jobs


def fetch_publication(job, output_format, debug, session):
    match = BNA_URL_PATTERN.match(job.url)
    if not match:
    ...
    if not loaded_pages:
        ...
        page_filter = build_page_filter(initial_url_suffix, job.page_ranges)
    ...
    fetch_pages(pages, output_format, session)
